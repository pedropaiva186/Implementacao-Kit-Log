...
std::vector<int> preencheCL(int& num) {
    Data &data = Data::getInstance();
    std::vector<int> c;
...
// Função que gerará a solução anterior ao uso do Grasp
int gerarInicio(Solution& s) {
    Data &data = Data::getInstance();

    // Gerando a solução inicial apenas com {1, 1}
    s.route = {1, 1};

    // Escolhendo vértice aleatório
    int vertice = rand() % (data.n - 1) + 2;

    // Inserindo o vértice na rota
    s.route.insert(s.route.begin() + 1, vertice);

    // Incrementando o custo
    s.cost = 2 * data.matrizAdj[1][vertice];
...
Solution Construcao() {
    ...
    int verticeAleatorio = gerarInicio(s);
    std::vector<int> CL = preencheCL(verticeAleatorio);
